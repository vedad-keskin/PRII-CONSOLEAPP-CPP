...
public:
    ...
    bool AddOcjena(Datum datum, int ocjena) {
        ...
        {
            if (datum.SumaDana() < _ocjeneRjesenja.getElement2(i)->SumaDana() || 
                datum.SumaDana() - _ocjeneRjesenja.getElement2(i)->SumaDana() < 3 ) {
                cout << "Nije dozvoljeno dati ocjenu na taj datum" << endl;
                return false;
            }
        }
    ...
    friend ostream& operator<< (ostream& COUT, const Pitanje& obj) {
        ...
        COUT << "Prosjek pitanja -> " << obj.GetProsjek() << endl;
        return COUT;
    }
};
class Ispit {
